...
# Load data
iddfs = glob.glob('*-ids.csv')
originaldfs = glob.glob('*[0-9]s.csv')

for iddf in iddfs:
    df = pd.read_csv(iddf)
    msID = iddf.split('s.csv')[0][-4:]
    orig_df = pd.read_csv([f for f in originaldfs if msID in f][0])
    ...
    interpolated_df.to_csv(iddf+'-postprocessed.csv',
                        index=False)